...
namespace XDomainProperties
{
   ...
   inline bool operator==(const uuid& lhs, const uuid& rhs){ return lhs.m_l == rhs.m_l && lhs.m_h == rhs.m_h; }
   inline bool operator!=(const uuid& lhs, const uuid& rhs){ return !operator==(lhs, rhs); }
   ...
   public:
      RootDirectoryHeader(uint32_t busType, uint8_t version) :m_BusType(busType),
         m_Version(version)
      {
      ...
      friend inline bool operator==(const RootDirectoryHeader& lhs, const RootDirectoryHeader& rhs){ return lhs.m_BusType == rhs.m_BusType && lhs.m_Version == rhs.m_Version; }
      friend inline bool operator!=(const RootDirectoryHeader& lhs, const RootDirectoryHeader& rhs){ return !operator==(lhs, rhs); }
   private:
   ...
   */
   ...
   public:
      ...
      friend inline bool operator==(const RootDirectory& lhs, const RootDirectory& rhs){ return lhs.m_Data == rhs.m_Data && lhs.m_Header == rhs.m_Header; };
   ...
   public:
      Property& operator[](const std::string& key);
      const Property& operator[](const std::string& key)const;
      friend inline bool operator==(const Properties& lhs, const Properties& rhs){ return lhs.m_RooDirectory == rhs.m_RooDirectory; };
   ...
   private:
      ...
      static void ParseProperty(std::istream& stream, const XDomainProperties::KeyValueEntry& propertyEntry, XDomainProperties::Property& prop);
      static void ReadDirectory(std::istream &stream, const XDomainProperties::KeyValueEntry &propertyEntry, XDomainProperties::Property& prop);
      static void ReadData(std::istream &stream, const XDomainProperties::KeyValueEntry &propertyEntry, XDomainProperties::Property& prop);
      ...
      static void WriteData(std::vector<uint8_t>& buffer, uint32_t& offset, KeyValueEntry& entry, const std::vector<uint32_t>& data, uint32_t& DataOffsetInDw);
      static void WriteDirectory(std::vector<uint8_t>& buffer, uint32_t& offset, KeyValueEntry& entry, ChildDirectory dir, uint32_t& DataOffsetInDw);
      static void WriteValue(std::vector<uint8_t>& buffer, uint32_t& offset, KeyValueEntry& entry, int32_t data);
      static void WriteText(std::vector<uint8_t>& buffer, uint32_t& offset, KeyValueEntry& entry, std::string data, uint32_t& DataOffsetInDw);
      ...
      static void WritePropertyToBuffer(const std::pair<const std::string, Property>& keyValuePair, std::vector<uint8_t>& buffer, uint32_t& offset, uint32_t& DataOffsetInDw);
